...
const sortByChildren = (charArray) => {
  // Solution code here...
 const ch = charArray.sort((a, b) => ( a.children.length - b.children.length ));
  return ch;
};
...
const getCourseKeys = (obj) => {
  // Solution code here...
  return (obj = Object.keys(courseInfo));
};
...
const checkValues = (obj, value) => {
  // Solution code here...
  const ob = Object.values(obj).includes(value);
  return ob;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const array = arr.filter(e =>( e.name === character ))
  if(array.length === 0){
    return false;
  }
  else{
...
const totalCharacters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].spouse) totChar += 1;
    if (arr[i].name) totChar += 1;
    totChar += arr[i].children.length;
  }
  return totChar;
};
...
const houseSize = (arr) => {
  const sizes = [];
  // Solution code here...
  arr.forEach((e) =>(sizes.push({ house: e.house, members: e.children.length + 1 }))
  );
  return sizes;
};
